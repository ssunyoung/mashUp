		function reload() { //새로고침을 위한 함수 -> 밑에 refresh버튼과 연관
		...
		function makeTable(h,m) { //셔틀버스 남는 시간 계산 함수
			...
			if( h>=8 && h<13) { //셔틀버스 정보가 뜨는 시간(8시~ 13시)
				...
				alert2 = '약 ' + diff +'분 후 출발</br>'; //해당 시간차(diff)를 받아 남는 시간 문장 작성
			}	
			else {
				alert2 = '등교시간 셔틀버스는 종료되었습니다.</br>'; //13시 이후에는 제공하지 않음
			}							
		// document.getElementById('shuttle').innerHTML=alert2;
		//$('#shuttle').append(alert2); //남는 시간 문장을 html 코드에 붙여줌.
