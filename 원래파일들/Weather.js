...
{   
      ...
      { 
          ...
          let today_day = days[today_date.getDay()];
           ...
           //현재 온도 클래스에 데이터 저장
          CURRENT_TEMP.innerHTML = `${Math.round(dailyForecast[0].temp.day)+"º"}<i class="wi ">${"<img src='"+applyIcon(dailyForecast[0].weather[0].icon)+"'>"}</i>
           <i class="wi wi-degrees"></i>`; 
          //현재 위치 클래스에 데이터 저장
          CURRENT_LOCATION.innerHTML = `<h1 id="today">${today_day+" "+today_month+"/"+today__date}</h1> <i id="current_location_h1">${city.name}</i><br>
